...
def process_first_steps(*, brief, examples_dir, rubric_notes, dry_run):
    ...
    system = load_base_system_prompt()
    if dry_run:
        return first_steps_rubric.first_steps_json_template()
    raw = call_claude_json(agent_dir=AGENT_DIR, system_prompt=system, user_prompt=user)
    return first_steps_rubric.normalize_first_steps_payload(raw)
...
def save_first_steps_output(data, *, out_path, jira_url):
    wb = first_steps_rubric.build_first_steps_workbook(data, jira_url=jira_url)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    wb.save(out_path)
...
def upload_to_sheets(rubric: str, data: Any, *, title: str, jira_url: str) -> str | None:
    ...
    from rubrics.first_steps import ROW_ORDER as FIRST_STEPS_ROWS
    ...
    try:
        ...
        elif rubric == "First steps in trading":
...
def process_one_brief(
    *, brief: dict[str, Any], rubric: str, dry_run: bool, cfg: dict[str, Any],
    ...
    if rubric == "Strategy":
        ...
        data = process_first_steps(brief=brief, examples_dir=examples_dir, rubric_notes=notes, dry_run=dry_run)
        if not dry_run:
